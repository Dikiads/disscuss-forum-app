    ...
    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
      ...
      <footer className="flex items-center justify-between border-t border-gray-100 pt-4">
        <div className="flex items-center gap-4 text-sm text-gray-600">
          <button
            onClick={() => upVoteThread(isThreadUpVoted)}
            className={`flex items-center gap-1 hover:text-blue-600 transition-colors ${isThreadUpVoted ? 'text-blue-600' : ''}`}
          >
            <i
              className={`${isThreadUpVoted ? 'fa-solid' : 'fa-regular'} fa-thumbs-up`}
            ></i>{' '}
            {threadDetail.upVotesBy.length}
          </button>
          <button
            onClick={() => downVoteThread(isThreadDownVoted)}
            className={`flex items-center gap-1 hover:text-red-500 transition-colors ${isThreadDownVoted ? 'text-red-500' : ''}`}
          >
            <i
              className={`${isThreadDownVoted ? 'fa-solid' : 'fa-regular'} fa-thumbs-down`}
            ></i>{' '}
